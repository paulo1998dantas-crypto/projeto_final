...
# Importações locais (Certifique-se que database.py e models.py estão na mesma pasta)
...
ETAPAS_PRODUCAO = ["VIDRO", "A/C", "SERRALHERIA", "ELÉTRICA", "ISOLAMENTO", "REVESTIMENTO", "ACESSORIO", "LIBERAÇÃO"]
...
async def home(request: Request, db: Session = Depends(database.get_db), modelo: str = None, etapa: str = None):
    ...
    for v in veiculos_db:
        ...
        apontamentos = db.query(models.Apontamento).filter(
            func.trim(cast(models.Apontamento.chassi, String)) == chassi_key).all()
        status_map = {str(a.etapa).strip().upper(): str(a.status).strip().upper() for a in apontamentos}

        concluidos = sum(1 for s in status_map.values() if s in ["SIM", "S", "OK", "N/A"])
    ...
    return templates.TemplateResponse("index.html",
                                      {"request": request, "veiculos": veiculos_exibicao, "etapas": ETAPAS_PRODUCAO,
...
async def detalhes(request: Request, chassi: str, db: Session = Depends(database.get_db)):
    ...
    veiculo = db.query(models.Veiculo).filter(func.trim(cast(models.Veiculo.chassi, String)) == c_limpo).first()
    feitos = db.query(models.Apontamento).filter(func.trim(cast(models.Apontamento.chassi, String)) == c_limpo).all()
    status_map = {str(f.etapa).strip().upper(): str(f.status).strip().upper() for f in feitos}
    return templates.TemplateResponse("detalhes.html",
                                      {"request": request, "veiculo": veiculo, "etapas": ETAPAS_PRODUCAO,
...
async def upload_base(file: UploadFile = File(...), db: Session = Depends(database.get_db)):
    try:
        ...
        for _, row in df.iterrows():
            ch_raw = str(row['CHASSI']).strip().split('.')[0]
            if not ch_raw or ch_raw.lower() == "nan": continue
            modelo = str(row['MMMV']).strip().upper()
            db.add(models.Veiculo(chassi=ch_raw, modelo=modelo))
            for etapa in ETAPAS_PRODUCAO:
                if etapa in df.columns:
                    val = str(row[etapa]).upper().strip()
                    status = "SIM" if val in ["S", "SIM", "OK"] else "NÃO" if val in ["N", "NÃO", "X"] else "N/A"
                    db.add(models.Apontamento(chassi=ch_raw, etapa=etapa, status=status))
        db.commit()
...
async def salvar(data: dict = Body(...), db: Session = Depends(database.get_db)):
    ch, et, st = str(data['chassi']).strip(), str(data['etapa']).strip().upper(), str(data['status']).strip().upper()
    db.query(models.Apontamento).filter(func.trim(cast(models.Apontamento.chassi, String)) == ch,
                                        func.trim(cast(models.Apontamento.etapa, String)) == et).delete()
    db.add(models.Apontamento(chassi=ch, etapa=et, status=st))
    v = db.query(models.Veiculo).filter(func.trim(cast(models.Veiculo.chassi, String)) == ch).first()
    db.add(models.Historico(chassi=ch, modelo=v.modelo if v else "N/A", etapa=et, status=st))
    db.commit()
    return {"status": "ok"}
...
async def exportar(db: Session = Depends(database.get_db)):
    ...
    if not logs: return {"message": "Sem dados"}
    df = pd.DataFrame(
        [{"CHASSI": l.chassi, "MODELO": l.modelo, "ETAPA": l.etapa, "STATUS": l.status, "DATA": l.data_apontamento} for
         l in logs])
    out = io.BytesIO()
